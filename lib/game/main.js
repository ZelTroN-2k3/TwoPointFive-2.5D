...
// MODIFICATION : On prend la taille de la fenêtre par défaut
	var width = window.innerWidth;
	var height = window.innerHeight;

	// Fonction pour redimensionner le jeu dynamiquement
	var resizeGame = function() {
		// Ajuste le canvas et le moteur de jeu
		if( ig.system ) {
			ig.system.resize( window.innerWidth, window.innerHeight );
		}
		// Repositionne les contrôles tactiles si nécessaire
		if( ig.game && ig.game.setupTouchControls ) { 
			ig.game.setupTouchControls(); 
		}
	};

	// On écoute le changement de taille de la fenêtre (PC et Mobile)
	window.addEventListener('resize', function(){
		setTimeout( resizeGame, 16 );
	}, false);

	// Gestion spécifique Ejecta (reste inchangée si vous l'utilisez)
	if( window.Ejecta ) {
	...
	if( ig.System.hasWebGL() ) {
		// MODIFICATION : On passe les variables dynamiques width/height
